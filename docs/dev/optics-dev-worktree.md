...
This guide establishes the standard workflow for developing Sigil alongside
...
|---------|---------------------------|------------------------------|
| Directory path | `../sigil/` | `../sigil-dev/` |
...
A feature compiling and passing tests in `sigil-dev` is often a **local
...
directory (used by `sigil-prompter`).
...
1. **Local Patch Containment**: `.cargo/config.toml` and path-resolved
   `Cargo.lock` belong strictly to the `sigil-dev` worktree. The tracked
...
```text
projects/
├── sigil/         # Primary worktree on main (canonical)
├── sigil-dev/     # Linked development worktree on dev (local patch)
└── optics/        # Sibling checkout of the Optics C engine
```

### 1. Create the Linked Worktree

Run from the primary `sigil` repository (`../sigil/`, branch `main`):

```bash
# In projects/sigil/ (branch main):
...
```bash
# In projects/sigil-dev/ (branch dev):
cd ../sigil-dev
...
Run an initial check in `sigil-dev` to populate the local worktree
...
> development worktree (`../sigil-dev/`, branch `dev`), interacting with
> the sibling directory `../optics/`.
>
> Do not run these daily development commands in the primary `sigil/`
> (`main`) worktree.

### Compiling and Testing

When modifying Optics and Sigil simultaneously:
...
1. Recompile the native C libraries whenever Optics C sources change:
   ...
   meson compile -C ../optics/build
   ```
2. Build and test Sigil in the development worktree. The `-sys` crates
...
# In projects/sigil-dev/ (branch dev):
git add .
git commit -m "feat(prompter): update prompt interaction"
```
...
The commit proceeds with only your Sigil source changes. Never bypass this
...
Land and tag the required changes in `../optics` before making Sigil
canonical:
...
Switch to the primary `sigil/` worktree (`branch main`). Because `main`
...
```bash
# In projects/sigil/ (primary worktree, branch main):
cd ../sigil
git switch main
git pull --ff-only
git merge dev
...
in `projects/sigil/Cargo.toml` to the new tag `vX.Y.Z`.
...
Notice that **`sigil-dev` never touched or disabled its `.cargo/config.toml`**.
...
```bash
# In projects/sigil-dev/ (development worktree, branch dev):
cd ../sigil-dev
...
#### 1. In Local Optics Mode (`sigil-dev/`)
...
- **Automatic Path Unstaging**: During regular feature commits, the hook
  ...
  ```
  The commit proceeds with only your clean Sigil source changes.
- **Refusing Release-Shaped Commits**: If you attempt to commit a version bump
  ...
  directly inside the primary main worktree (../sigil/).
  ```

#### 2. In Canonical Mode (`sigil/`, `main`)
...
```bash
# In projects/sigil/ (main) or projects/sigil-dev/ (dev):
...
```bash
# 1. Pull the primary worktree (in projects/sigil/, branch main):
cd ../sigil
git pull --ff-only

# 2. Rebase the development worktree (in projects/sigil-dev/, branch dev):
cd ../sigil-dev
git restore Cargo.lock
git rebase main
cargo check --workspace
